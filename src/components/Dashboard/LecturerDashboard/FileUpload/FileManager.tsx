...
export const FileManager: React.FC<FileManagerProps> = ({
  ...
  useEffect(() => {
    loadFiles();
  }, [context.courseId, context.moduleId, context.topicId, context.type]);
      ...
      <div className={`space-y-4 ${className}`}>
          ...
          <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto justify-between sm:justify-end">
            ...
            <div className="flex bg-gray-100 rounded-lg p-0.5 sm:p-1">
              <button
                ...
                className={`px-2 sm:px-3 py-1 rounded-md text-xs sm:text-sm font-bold transition-colors ${viewMode === 'grid' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600'
                  }`}
              ...
              <button
                ...
                className={`px-2 sm:px-3 py-1 rounded-md text-xs sm:text-sm font-bold transition-colors ${viewMode === 'list' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600'
                  }`}
        ...
        {files.length === 0 ? (
          ...
          <div className={
            ...
            {files.map((file) => {
              return (
                <div key={file.id} className={`bg-white rounded-xl border border-gray-200 transition-all hover:shadow-md ${selectedFiles.has(file.id) ? 'ring-2 ring-blue-500 bg-blue-50' : ''
